...
export default function LuxuryVisualLoungeApp() {
  ...
  const [loyaltyStamps, setLoyaltyStamps] = useState<number>(6);
  const maxStamps = 9;
  ...
  const t = {
    EN: {
      ...
      checkoutTab: "Checkout Register",
      medinaBoardTab: "Medina Board",
      anisWaiting: "Anis is online and ready for your order customizations... ✨",
      ...
      cashActive: "● Cash Settle Node: Active",
    ...
    FR: {
      ...
      ritualCircle: "Récompenses Rituelles",
      clickToOrder: "Cliquez pour Configurer",
      slowBrew: "Extraction Douce & Pour Overs",
      ...
      anisWaiting: "Anis est en ligne et prêt pour vos personnalisations de commande... ✨",
      ...
      fiberActive: "● Connexion : Fibre 1 Gbps active",
      cashActive: "● Nœud d'espèces : Activé",
                ...
                {customizer.type === 'drink' && (
                  <div className="space-y-4">
                    <div className="space-y-1.5">
                      <label className="block text-[0.6rem] tracking-[0.12em] uppercase text-[#6B6460] font-medium">{t[lang].milkOptionsLabel}</label>
          ...
          <header className="max-w-7xl w-full mx-auto flex justify-between items-center border-b border-[#D5CFC4] pb-4 mb-6">
            ...
            <div className="flex items-center gap-4">
              <div className="text-xs uppercase tracking-widest font-medium border border-[#D5CFC4] rounded-[2px] p-1 flex gap-2 bg-[#EDEBE3]/50">
                <button onClick={() => setLang('EN')} className={`px-1.5 py-0.5 rounded-[1px] ${lang === 'EN' ? 'bg-[#1A1714] text-[#F5F0E8]' : 'text-[#6B6460]'}`}>EN</button>
                <button onClick={() => setLang('FR')} className={`px-1.5 py-0.5 rounded-[1px] ${lang === 'FR' ? 'bg-[#1A1714] text-[#F5F0E8]' : 'text-[#6B6460]'}`}>FR</button>
              </div>
              <button onClick={() => setShowChat(false)} className="text-[0.65rem] tracking-[0.15em] uppercase border border-[#D5CFC4] px-3 py-1.5 rounded-[2px] bg-transparent">{t[lang].exitBtn}</button>
            </div>
          </header>

          {/* MASTER GRID CONTROLLER */}
          <div className="flex-1 max-w-7xl w-full mx-auto grid grid-cols-1 lg:grid-cols-4 gap-6 items-start overflow-hidden pb-4">
            
            {/* ☕ HIGH-END VISUAL CATALOG SIDEBAR PANEL */}
            <aside className="lg:col-span-1 bg-[#EDEBE3] border border-[#D5CFC4] rounded-[2px] p-4 space-y-6 max-h-[52vh] lg:max-h-[82vh] overflow-y-auto style-scrollbar">
              ...
              {/* Rewards node wrapper */}
              <div className="bg-[#1A1714] text-[#F5F0E8] p-4 rounded-[2px] space-y-2">
                <div className="flex justify-between items-center text-[0.6rem] uppercase tracking-widest"><span className="text-[#C9A96E] font-medium">{t[lang].ritualCircle}</span><span>{loyaltyStamps}/{maxStamps}</span></div>
              ...
              <div className="space-y-5">
                {/* Visual Category 1: Pour Overs */}
                <div className="space-y-2.5">
                  <h3 className="text-[0.58rem] tracking-[0.15em] uppercase text-[#8A9E8C] font-semibold">{t[lang].slowBrew}</h3>
                  <div className="relative w-full h-24 rounded-[1px] overflow-hidden group border border-[#D5CFC4]/60">
                    <img src="https://images.unsplash.com/photo-1514432324607-a09d9b4aefdd?w=400&auto=format&fit=crop&q=80" alt="Slow Brew" className="absolute inset-0 w-full h-full object-cover grayscale transition-all duration-500 group-hover:grayscale-0 group-hover:scale-105" />
                  ...
                  <div className="space-y-1.5 text-xs">
                    <button onClick={() => openCustomizer('V60 Ethiopia Light Crop', 'V60 Éthiopie Torréfaction Claire', 45, 'drink')} className="w-full flex justify-between bg-transparent border-none p-0 text-left hover:text-[#B8734A] cursor-pointer"><span>Ethiopia V60 Single Origin</span><span className="font-medium">45 MAD</span></button>
                    <button onClick={() => openCustomizer('Chemex Colombia Geisha', 'Chemex Colombie Geisha', 55, 'drink')} className="w-full flex justify-between bg-transparent border-none p-0 text-left hover:text-[#B8734A] cursor-pointer"><span>Chemex Colombia Geisha</span><span className="font-medium">55 MAD</span></button>
                  </div>
                </div>

                {/* Visual Category 2: Signatures */}
                <div className="space-y-2.5">
                  <h3 className="text-[0.58rem] tracking-[0.15em] uppercase text-[#8A9E8C] font-semibold">{t[lang].houseSignatures}</h3>
                  <div className="relative w-full h-24 rounded-[1px] overflow-hidden group border border-[#D5CFC4]/60">
                    <img src="https://images.unsplash.com/photo-1570968915860-54d5c301fc9f?w=400&auto=format&fit=crop&q=80" alt="Signatures" className="absolute inset-0 w-full h-full object-cover grayscale transition-all duration-500 group-hover:grayscale-0 group-hover:scale-105" />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent flex flex-col justify-end p-2.5" />
                  </div>
                  <div className="space-y-1.5 text-xs">
                    <button onClick={() => openCustomizer('Signature Rose & Cardamom Latte', 'Latte Artisanal Rose & Cardamome', 45, 'drink')} className="w-full flex justify-between bg-transparent border-none p-0 text-left hover:text-[#B8734A] cursor-pointer"><span>Rose Cardamom Latte</span><span className="font-medium">45 MAD</span></button>
                    <button onClick={() => openCustomizer('Orange Blossom Espresso Shakerato', 'Shakerato Fleur d’Oranger', 40, 'drink')} className="w-full flex justify-between bg-transparent border-none p-0 text-left hover:text-[#B8734A] cursor-pointer"><span>Orange Blossom Shakerato</span><span className="font-medium">40 MAD</span></button>
                  </div>
                </div>

                {/* Visual Category 3: Pastries */}
                <div className="space-y-2.5">
                  <h3 className="text-[0.58rem] tracking-[0.15em] uppercase text-[#8A9E8C] font-semibold">{t[lang].artisanalBakery}</h3>
                  <div className="relative w-full h-24 rounded-[1px] overflow-hidden group border border-[#D5CFC4]/60">
                    <img src="https://images.unsplash.com/photo-1555507036-ab1f4038808a?w=400&auto=format&fit=crop&q=80" alt="Bakery" className="absolute inset-0 w-full h-full object-cover grayscale transition-all duration-500 group-hover:grayscale-0 group-hover:scale-105" />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent flex flex-col justify-end p-2.5" />
                  </div>
                  <div className="space-y-1.5 text-xs">
                    <button onClick={() => openCustomizer('Pistachio Stuffed Croissant', 'Croissant Fourré Pistache', 45, 'pastry')} className="w-full flex justify-between bg-transparent border-none p-0 text-left hover:text-[#B8734A] cursor-pointer"><span>Pistachio Croissant</span><span className="font-medium">45 MAD</span></button>
                    <button onClick={() => openCustomizer('San Sebastián Sea-Salt Honey Cake', 'Gâteau de Miel San Sebastián', 50, 'pastry')} className="w-full flex justify-between bg-transparent border-none p-0 text-left hover:text-[#B8734A] cursor-pointer"><span>San Sebastián Honey Cake</span><span className="font-medium">50 MAD</span></button>
                  </div>
                </div>
              </div>
            </aside>

            {/* INTERACTIVE COMPONENT WORKSPACE MATRIX */}
            <div className="lg:col-span-2 flex flex-col bg-[#FDFCF9] border border-[#D5CFC4] rounded-[2px] p-5 h-[72vh] lg:h-[82vh] justify-between">
              ...
              <div className="flex-1 overflow-y-auto pr-1 style-scrollbar">
                ...
                {/* SUBPANEL 1: ACTIVE ANIS CHAT INTERACTION */}
                {activeTab === 'chat' && (
                  <div className="space-y-4 h-full flex flex-col justify-between">
                    <div className="space-y-4 overflow-y-auto flex-1 max-h-[52vh]">
                      {messages.length === 0 ? (
                        <div className="text-center py-20 space-y-2">
                          <p className="font-['Cormorant_Garamond'] italic text-2xl text-[#6B6460]">"Salam Alaykum, {guestName || 'Friend'}."</p>
                          <p className="text-[0.68rem] tracking-[0.12em] uppercase text-[#8A9E8C]">{t[lang].anisWaiting}</p>
                        </div>
                      ) : (
                        messages.map((m, i) => (
                      ...
                      {isLoading && <p className="text-[0.65rem] tracking-[0.15em] uppercase text-[#8A9E8C] animate-pulse">{t[lang].anisPouring}</p>}
                    </div>
                ...
                {/* 🏰 HIGH QUALITY VISUAL EXPERIENCES & SPACE BOOKING PANELS */}
                {activeTab === 'spaces' && (
                      ...
                      <div className="border border-[#D5CFC4] bg-[#EDEBE3]/30 rounded-[1px] overflow-hidden flex flex-col group">
                        ...
                        <div className="p-4 flex-1 flex flex-col justify-between space-y-3">
                          <div>
                            ...
                            <p className="text-[0.68rem] text-[#6B6460] font-light mt-1 leading-relaxed">Fiber grid connectivity array, acoustic isolation panels, unmetered artisanal reverse osmosis water streams.</p>
                          </div>
                ...
                {/* SUBPANEL 3: INVOICING CHECKOUT MATRIX */}
                {activeTab === 'checkout' && (
  ...
  // ─── VIEW 2: SIGN-IN PORTAL GRID WITH SEAMLESS BILINGUAL IMAGE RE-RENDERING ───
  return (
    <>
      ...
      {/* Language controls right at entryway door panel */}
      ...
      <main className="min-h-screen grid grid-cols-1 md:grid-cols-2 bg-[#F5F0E8] text-[#1A1714] font-['DM_Sans',_sans-serif] font-light antialiased">
        ...
        {/* LEFT ATMOSPHERE HALF — IMMERSIVE SPLIT SCREEN */}
        <section className="relative bg-[#1A1714] overflow-hidden flex flex-col justify-end p-8 md:p-12 min-h-[45vh] md:min-h-screen group">
          ...
          {/* Main high quality visual lifestyle layer running silently behind the geometric line grids */}
          <div className="absolute inset-0 z-0">
            <img 
              src="https://images.unsplash.com/photo-1541167760496-1628856ab772?w=1000&auto=format&fit=crop&q=80" 
              alt="Luxury Espresso Pouring" 
              className="w-full h-full object-cover opacity-20 grayscale scale-100 transition-transform duration-1000 group-hover:scale-105"
            />
          </div>
          ...
          <div className="relative z-20 space-y-6 animate-[fadeUp_0.9s_ease_both] w-full">
            ...
            {/* Login Active Wall Member Mosaic */}
            <div className="space-y-2.5">
                ...
                {activeWallMembers.map((m) => (
                  <div key={m.id} className="relative group/avatar flex flex-col items-center">
                    ...
                    <div className="absolute -top-7 bg-[#1A1714] text-[#F5F0E8] text-[0.55rem] px-1.5 py-0.5 rounded-[1px] border border-[#C9A96E]/30 opacity-0 group-hover/avatar:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-30">
                      {m.name} · <span className="text-[#8A9E8C]">{lang === 'EN' ? m.statusEN : m.statusFR}</span>
                    </div>
                  </div>
                ))}
              </div>
